...
- **Per-Tracker Image Hosts** – RED covers go to RED's own image host, which is for album artwork only; everything else uses a neutral host, and spectrals never go to RED's host. Since OPS proxies and caches RED-hosted images, a RED cover already uploaded in the same run is reused as-is for a new OPS group instead of uploading it again, and RED to OPS cross-uploads keep RED-hosted images in the cover and both descriptions the same way.
