...
- **Per-Tracker Image Hosts** – each tracker can use its own cover host. RED's image host is for album artwork only, so it is accepted solely as `cover_uploader` under `[image.red]` and refused for description images, spectrals and other trackers' covers. Opt in with `[image.red]` and `cover_uploader = "red"`; without it RED covers follow the global host like everything else. When RED covers do go to RED, a cover uploaded earlier in the same run is reused as-is for a new OPS group rather than uploaded again, since OPS proxies and caches RED-hosted images, and RED to OPS cross-uploads keep them the same way.
...
```
This only works from a lossless FLAC (anything else stops before a file is touched), and uploads to that one
tracker. The usual checks still run, on a scratch copy in `download_directory/.salmon-staging` that is removed when
the run ends: the source folder is never modified, since it is likely seeding the FLAC. The transcodes land in
`download_directory` as for any upload. The transcode
