...
- **Frequency Analysis** – One averaged-spectrum curve per track, plus the two measurements that tell a lossy encoder from a master: a brick-wall lowpass in the band where MP3 and AAC encoders cut (16–20.5 kHz), and highs that flip between content and the bit-depth floor while the music plays, which is an encoder running short of bits. The cutoff alone raises nothing — honest masters roll off early, and every 44.1 kHz master has a wall near 21 kHz from sample-rate conversion — so a track is flagged only on the marks, and a compilation whose tracks stop at different frequencies is left alone.  
