...
def test_search_reports_inactive_without_an_app_id(monkeypatch) -> None:
    ...
    assert anyio.run(Searcher().search_releases, "illy journaling", 5) == ("Qobuz", None)
...
def test_headers_omit_unset_credentials(monkeypatch) -> None:
...
def test_configured_search_sends_both_headers(monkeypatch) -> None:
    ...
    assert anyio.run(Searcher().search_releases, "illy journaling", 5) == ("Qobuz", {})
