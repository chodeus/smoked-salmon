...
from salmon.common import commandgroup, decade_tag, get_audio_files, tagify
...
from salmon.tagger.pre_data import construct_rls_data, parse_format
...
# Scratch copies of --skip-flac-upload sources live here, one directory per run, removed when it ends.
...
def _is_flac_release(path: str) -> bool:
    """Whether every audio file in the folder is a FLAC."""
    files = get_audio_files(path)
    return bool(files) and all(parse_format(f) == "FLAC" for f in files)
...
async def _new_group_cover_url(
    site_code: str, stored_cover_urls: dict[str, str], path: str, cover_source: str | None, remove_downloaded: bool
) -> str | None:
    """Cover URL for a new group on this tracker, uploading it unless its host already has it."""
    ...
    return stored_cover_urls.get(cover_host)


async def resolve_cover_url(
    site_code: str,
    group_id: int | None,
    stored_cover_urls: dict[str, str],
    path: str,
    cover_source: str | None,
    remove_downloaded: bool,
) -> tuple[bool, str | None]:
    """Whether to upload to this tracker, and the cover URL; asks before a new group goes up without one."""
    if group_id:
        if not remove_downloaded:
            await download_cover_if_nonexistent(path, cover_source)
        return True, None

    cover_url = await _new_group_cover_url(site_code, stored_cover_urls, path, cover_source, remove_downloaded)
    if cover_url:
    ...
    while True:
        raw_choice = await click.prompt(
            click.style("Continue upload without a cover image? [y/N/reload]", fg="magenta"),
            default="n",
            show_default=False,
        )
        ...
        if cover_url:
            ...
            return True, cover_url
        click.secho("Still no cover image found. Add a cover.jpg to the release folder and try again.", fg="yellow")
...
async def upload(
    ...
    if flac_group is not None and not _is_flac_release(path):
        return click.secho(
            f"\n--skip-flac-upload only uploads transcodes of a lossless FLAC, and {path} holds other audio.",
            fg="red",
            bold=True,
        )
    ...
    try:
        if flac_group is not None:
            # A scratch copy: never uploaded or seeded, so it goes when the run ends.
            scratch_dir = _new_scratch_dir()
            path = _stage_source(path, scratch_dir)
        elif cfg.directory.is_library_path(path):
        ...
        try:
            ...
            if scratch_dir:
                click.secho(
                    f"\nNot deleting {source_path}: with --skip-flac-upload the source is never modified.",
                    fg="yellow",
                    bold=True,
                )
                return click.secho("\nAborting upload...", fg="red")
        ...
        if scratch_dir:
            _remove_scratch_dir(scratch_dir, source_path)
